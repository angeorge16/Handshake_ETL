USE [handshakeDB];  -- replace with your DB
GO
...
BEGIN
    ...
    BEGIN
	    IF @mode = 'delta'
		BEGIN
			SELECT * FROM dbo.delta_rows;
            RETURN;
		END
		ELSE IF @mode = 'full'
		BEGIN
			SELECT * FROM dbo.previous_run_test;
            RETURN;
		END
    END
...
-- 1) Create temp table for today's run
    ...
    -- union current and future student data (replace with your actual queries)
    ...
    SELECT 
        ...
        dbo.fn_ConcatMajors_v2(termc.STUDENT_CURR_1_MAJOR_NAME, termc.STUDENT_CURR_1_MAJOR_2_NAME, termc.STUDENT_CURR_2_MAJOR_1_NAME, termc.STUDENT_CURR_2_MAJOR_2_NAME) AS primary_education_major_names,
        dbo.fn_ConcatMajors_v2(termc.STUDENT_CURR_1_MINOR_1_NAME, termc.STUDENT_CURR_1_MINOR_2_NAME, termc.STUDENT_CURR_2_MINOR_1_NAME, termc.STUDENT_CURR_2_MINOR_2_NAME) AS primary_education_minor_names,
    ...
    SELECT 
        ...
        dbo.fn_ConcatMajors_v2(termf.STUDENT_CURR_1_MAJOR_NAME, termf.STUDENT_CURR_1_MAJOR_2_NAME, termf.STUDENT_CURR_2_MAJOR_1_NAME, termf.STUDENT_CURR_2_MAJOR_2_NAME) AS primary_education_major_names,
        dbo.fn_ConcatMajors_v2(termf.STUDENT_CURR_1_MINOR_1_NAME, termf.STUDENT_CURR_1_MINOR_2_NAME, termf.STUDENT_CURR_2_MINOR_1_NAME, termf.STUDENT_CURR_2_MINOR_2_NAME) AS primary_education_minor_names,
...
-- Create temp table with row_hash and timestamp
SELECT *,
...
FROM unioned_data;
    ...
    -----------------------------
    -- 2) Insert the newly added or updated rows
    -----------------------------
    TRUNCATE TABLE dbo.delta_rows;

	INSERT INTO dbo.delta_rows
	SELECT 
    t.email_address,
	t.username,
    t.auth_identifier,
    ...
    -----------------------------
    TRUNCATE TABLE previous_backup_test;
    INSERT INTO previous_backup_test SELECT * FROM previous_run_test;

    TRUNCATE TABLE previous_run_test;
    INSERT INTO previous_run_test SELECT * FROM #current_run;

	--------------------------------
    -- Step 4: Log this run
    --------------------------------
    INSERT INTO dbo.run_log (last_execution_date, run_type)
    VALUES (@today, @mode);
...
EXEC dbo.sp_get_delta_from_source @mode = 'full';




select * from dbo.run_log
