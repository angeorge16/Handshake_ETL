...
BEGIN
    ...
    BEGIN
	    IF @mode = 'delta'
		BEGIN
			SELECT * FROM [dbo].[HANDSHK_CURRENT_STUDENTS_DELTA];
            RETURN;
		END
		ELSE IF @mode = 'full'
		BEGIN
...
FROM unioned_data;
    ...
    INSERT INTO [dbo].[HANDSHK_CURRENT_STUDENTS_PREV_RUN_BACKUP] SELECT * FROM [dbo].[HANDSHK_CURRENT_STUDENTS_PREV_RUN];

    TRUNCATE TABLE [dbo].[HANDSHK_CURRENT_STUDENTS_PREV_RUN];
    INSERT INTO [dbo].[HANDSHK_CURRENT_STUDENTS_PREV_RUN] SELECT * FROM #current_run;

	--------------------------------
    -- Step 4: Log this run
    --------------------------------
    INSERT INTO [dbo].[HANDSHK_CURRENT_STUDENTS_RUN_LOG] (last_execution_date, run_type)
    VALUES (@today, @mode);
