...
BEGIN
    ...
    FROM DATAMART.DBO.DMG_CENSUS_DATA Z
    WHERE Z.TERM_CD = @CURR_TERM_CD;

    ----------------------------------------------------------------------
    -- [2] GPA
    ----------------------------------------------------------------------
    IF OBJECT_ID('DATAMART.DBO.HANDSHK_ALUM_GPA1') IS NOT NULL
        DROP TABLE DATAMART.DBO.HANDSHK_ALUM_GPA1;

    SELECT DISTINCT 
        D.EDW_PERS_ID, 
        D.GPA_LEVEL_CD AS LEVEL_CD,
        MAX(CONVERT(DECIMAL(10,2), D.LEVEL_GPA)) AS LEVEL_GPA
    INTO DATAMART.DBO.HANDSHK_ALUM_GPA1
    FROM DSPROD1..EDW.T_STUDENT_AH_LEVEL_GPA_HIST D
    ...
    ----------------------------------------------------------------------
    IF OBJECT_ID('DATAMART.DBO.HANDSHK_ALUM_REG_TST1') IS NOT NULL
        DROP TABLE DATAMART.DBO.HANDSHK_ALUM_REG_TST1;
    ...
    SELECT  
        ...
        T.STUDENT_LEVEL_CD
    INTO DATAMART.DBO.HANDSHK_ALUM_REG_TST1
    FROM DSPROD1..EDW.T_STUDENT_TERM T
    WHERE T.TERM_CD = @PREV_TERM_CD;

    ----------------------------------------------------------------------
    -- [4] NETID
    ----------------------------------------------------------------------
    IF OBJECT_ID('DATAMART.DBO.HANDSHK_ALUM_NETID1') IS NOT NULL
        DROP TABLE DATAMART.DBO.HANDSHK_ALUM_NETID1;

    SELECT DISTINCT 
        T.EDW_PERS_ID,
        T.NETID_PRINCIPAL
    INTO DATAMART.DBO.HANDSHK_ALUM_NETID1
    FROM DSPROD1..EDW.T_NETID T
    ...
    ----------------------------------------------------------------------
    IF OBJECT_ID('DATAMART.DBO.HANDSHK_ALUM1') IS NOT NULL
        DROP TABLE DATAMART.DBO.HANDSHK_ALUM1;

    SELECT DISTINCT 
        AL.EDW_PERS_ID,
        AL.GRAD_TERM_CD AS TERM_CD,
        AL.STUDENT_LEVEL_CD
    INTO DATAMART.DBO.HANDSHK_ALUM1
    FROM DSPROD1..EDW.T_STUDENT_AH_DEG AL
